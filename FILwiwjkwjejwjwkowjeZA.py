...
RETURNS, TIK, TEL, INSTA ,FIVEINSTA,FOURTIK, FIVETIK, SIXTEL,FIVETEL = range(9)
...
def start(update: Update,context: CallbackContext):
    ...
    try:
        ...
        join = context.bot.get_chat_member(chat_id="@TweakPY", user_id=update.effective_chat.id)
        if (join.status == "kicked") or (join.status == "left"):
            update.message.reply_text(text=
"""عذرا عليك الاشتراك بقناه البوت اولا!
Sorry, you must subscribe in the bot channel 
first!
@TweakPY""")
        else:
            if IDDD in ID:
...
@TweakPY""")
            ...
            proxxx = ('[+] downloading the proxy now sending...✅')
            update.message.reply_text(proxxx)
            proxx = (re)
            update.message.reply_text(proxx)
    except telegram.error.BadRequest:
        msg = """
عذرا البوت لايعمل بالمجموعات!
Sorry, the bot does not work 
...
def help(update,context):
    tef="""THIS IS FREE BOT BY FILZA @Filza2 - @TweakPY\n\n
...
def main() -> None:
    ...
    okkk = CommandHandler('proxy', proxy,run_async=True)
    dp.add_handler(okkk)
    oksk = CommandHandler('help', help,run_async=True)
    dp.add_handler(oksk)
